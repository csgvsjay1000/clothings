...
import ivg.cn.clo.factory.db.config.sharding.tables.FDeliveryBillTable;
import ivg.cn.clo.factory.db.config.sharding.tables.FDeliveryBillTraceTable;
...
//		ruleConfiguration.getTableRuleConfigs().add(FDeliveryBillTable.build());
//		ruleConfiguration.getTableRuleConfigs().add(FDeliveryBillTraceTable.build());
//		ruleConfiguration.getBindingTableGroups().add("f_delivery_bill,f_delivery_bill_trace");  // 不配置也行
		ruleConfiguration.setDefaultDataSourceName("db_0");
	...
	private List<DSBaseConfig> loadDsConfigs() {
		...
		one.setUrl("jdbc:mysql://127.0.0.1:3306/db_0?characterEncoding=utf8&useSSL=true");
